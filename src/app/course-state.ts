...
const keys = { selected: "selectedCourses", favorite: "favoriteCourses", backup: "backupCourses", target: "creditTarget" };
...
export const initializeCourseState = () => {
  state.alternateCourseIds = read(keys.backup);
...
export const getBackupCourseIds = () => new Set(state.alternateCourseIds);
...
export const getBackupCourses = () => getCoursesByIds(getBackupCourseIds());
export const getCourseStats = () => {
  ...
  return {
    ...
    backupCount: getBackupCourses().length,
    selectedCredits,
...
export const isBackupCourse = (id: string) => state.alternateCourseIds.has(id);
...
export const toggleBackupCourse = (course: Course) => {
  state.alternateCourseIds.has(course.id) ? state.alternateCourseIds.delete(course.id) : state.alternateCourseIds.add(course.id);
  write(keys.backup, state.alternateCourseIds); emit();
};
export const persistBackupCourses = () => { write(keys.backup, state.alternateCourseIds); emit(); };
